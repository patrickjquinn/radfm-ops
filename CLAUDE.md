...
- **Identity now crosses to `/admin/*` via Access Linked App Token, not a bearer.** The verified assertion is forwarded as `Cf-Access-Token`; Access mints a new JWT scoped to the *Rad.FM Admin API* application's AUD (`71df112b…`) - **not** this dashboard's (`b01e1140…`). Confusing the two is what made every `/admin/*` route 404 on 6 Aug: valid signature, valid `iss`, an `aud` that could never match. Anything verifying that assertion downstream must expect Application B's AUD.

**Before deleting `OPS_BACKEND_JWT`, run `GET /api/probe/access`.** It calls `/admin/me` with the bearer deliberately suppressed and only the assertion forwarded, and passes only if a role comes back. "Confirm the dashboard still reaches `/admin/*`" is *not* that test - it passes with the bearer attached and proves nothing about the Access path. Acting on the weaker check took the dashboard down once. Note the probe is unreachable by browser navigation: SPA asset handling serves `index.html` for HTML navigations before the Worker runs, so fetch it as XHR.
